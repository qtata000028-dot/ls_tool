...
const CustomSelect = ({ value, onChange, options, placeholder = '请选择', icon: Icon, className = '' }: any) => {
  ...
  const selectedLabel =
    options.find((opt: any) => String(opt.value) === String(value))?.label || placeholder;
...
const ToolsPlatform: React.FC<ToolsPlatformProps> = ({ onBack, aiParams }) => {
  ...
  const localInsights = useMemo(() => {
    ...
    const deptCounts = employees.reduce<Record<string, number>>((acc, emp) => {
      const name =
        departments.find((d) => String(d.Departmentid) === String(emp.Departmentid))
          ?.departmentname || '未知部门';
    ...
    try {
      ...
      safeEmps.forEach((e: Employee) => {
        if (e.P_emp_workJoindt) {
          const year = String(e.P_emp_workJoindt).split('T')[0].substring(0, 4);
      ...
      console.error(err);
      setError(`数据加载失败: ${err?.message || String(err)}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateDynamicAnalysis = async (
    ...
    const keepAliveInterval = setInterval(() => {
      if (Math.random() > 0.6) {
        ...
        const hexAddr = `0x${Math.floor(Math.random() * 16777215)
          .toString(16)
          .toUpperCase()
          .padStart(6, '0')}`;
    ...
    const schemaKeys =
      Object.keys(activeSchema).length > 0 ? JSON.stringify(activeSchema) : 'Schema loading...';

    setTerminalLines((prev) => [
      ...prev,
      `[DATA] 成功加载 Schema 定义: ${Object.keys(activeSchema).length} 个字段映射.`,
    ]);

    try {
      const deptMappingSample = currentDepts
        .slice(0, 15)
...
Return JSON Format:
{
  ...
  "prediction": "Predict future based on the provided Recruitment Trend data. (e.g. '基于2023-2024年数据，招聘放缓...')"
      ...
      if (jsonMatch) {
        const config = JSON.parse(jsonMatch[0]) as AIReport;
      ...
      setTerminalLines((prev) => [...prev, `[ERROR] ${e?.message || String(e)}`]);
  ...
  const getAggregatedData = (chartConfig: ChartConfig) => {
    ...
    employees.forEach((emp) => {
      ...
      if (field === 'Departmentid') {
        val =
          departments.find((d) => String(d.Departmentid) === String(val))?.departmentname ||
          `Dept ${val}`;
      }
      if (!val) val = '未知';
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const sql = `UPDATE p_employeetab SET 
            employeename = '${sqlEscape(currentEmp.employeename)}',
            Departmentid = ${Number(currentEmp.Departmentid) || 0},
            P_emp_sex = '${sqlEscape(currentEmp.P_emp_sex)}',
            p_emp_phone = '${sqlEscape(currentEmp.p_emp_phone)}',
            P_emp_Status = '${sqlEscape(currentEmp.P_emp_Status)}',
            p_emp_degree = '${sqlEscape(currentEmp.p_emp_degree)}',
            P_emp_workJoindt = '${sqlEscape(currentEmp.P_emp_workJoindt ? String(currentEmp.P_emp_workJoindt).split('T')[0] : '')}'
            WHERE P_emp_no = '${sqlEscape(currentEmp.P_emp_no)}'`;
  ...
  const getDeptName = (id: string | number) =>
    departments.find((d) => String(d.Departmentid) === String(id))?.departmentname || `${id}`;
  ...
  return (
    <div className="w-full h-[85vh] max-w-[1600px] mx-auto bg-[#0F1629]/80 backdrop-blur-2xl border border-white/10 rounded-3xl flex flex-col overflow-hidden shadow-[0_30px_80px_-40px_rgba(0,0,0,0.8)] animate-in fade-in zoom-in-95 duration-500 relative">
      ...
      {/* Error Banner (避免 error state unused, 也方便你看 API 报错) */}
      {error && (
        <div className="px-6 py-3 text-xs text-red-200 bg-red-500/10 border-b border-red-500/20 flex items-center gap-2">
        ...
        <table className="w-full text-left border-separate border-spacing-0">
          <thead className="sticky top-0 z-20 bg-gradient-to-r from-[#0F1629] via-[#0F1629]/95 to-[#0F1629] text-xs font-bold text-slate-400 uppercase tracking-wider shadow-[0_10px_30px_-20px_rgba(0,0,0,0.8)] backdrop-blur">
            <tr>
              ...
              <th className="sticky right-0 z-30 bg-[#0F1629] px-6 py-4 border-b border-white/10 border-l border-white/5 text-right w-[100px]">
                操作
              </th>
            </tr>
          </thead>
                ...
                return (
                  ...
                  >
                    <td className="sticky left-0 z-10 px-6 py-3 bg-[#0F1629] group-hover:bg-[#131b2e] border-r border-white/5 transition-colors">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-slate-800 overflow-hidden flex items-center justify-center shrink-0 border border-white/10">
                          {row.webbmp ? (
                            <img src={row.webbmp} className="w-full h-full object-cover" />
                          ) : (
                    ...
                    <td className="px-6 py-3 text-slate-400 font-mono text-xs">
                      <div className="flex items-center gap-2 group/id">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            copyToClipboard(row.P_emp_no);
                          }}
                          className="opacity-0 group-hover/id:opacity-100 text-slate-500 hover:text-white"
                        >
                          <Copy size={12} />
                        </button>
                      </div>
                    ...
                    <td className="px-6 py-3">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded text-xs border ${getStatusColor(row.P_emp_Status)}`}
                      >
                        {row.P_emp_Status}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-[#0F1629] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95">
            ...
            <div className="p-6 grid grid-cols-2 gap-x-6 gap-y-5">
              ...
              <div className="col-span-1 space-y-1.5">
                ...
                <CustomSelect
                  ...
                  onChange={(val: any) => setCurrentEmp({ ...currentEmp, P_emp_Status: String(val) })}
            ...
            <div className="p-4 border-t border-white/5 bg-black/20 flex justify-end gap-3">
              <button
                onClick={() => setIsEditorOpen(false)}
                className="px-4 py-2 rounded-lg text-sm text-slate-400 hover:text-white hover:bg-white/5"
              >
                取消
              </button>
              <button
              ...
              <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
                ...
                {localInsights.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                    {localInsights.map((item) => (
                      <div
                        key={item.title}
                        className="p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md flex flex-col gap-2"
                      >
                        ...
                        <div className="text-sm text-white font-bold flex items-center gap-2">
                          <span className="px-2 py-1 rounded-full bg-indigo-500/10 text-[11px] text-indigo-200 border border-indigo-500/30">
                            {item.tag}
                          </span>
                          {item.title}
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {dataQualityMetrics.map((metric) => (
                    <div
                      key={metric.title}
                      className="p-4 rounded-2xl bg-[#0F1629]/60 border border-white/10 relative overflow-hidden"
                    >
                  ...
                  <div className="lg:col-span-1 space-y-6">
                    <div className="p-6 rounded-3xl bg-[#0F1629]/60 backdrop-blur-xl border border-white/5 relative overflow-hidden group">
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed border-t border-white/5 pt-3">
                        {aiReportConfig.prediction ||
                          'AI 模型分析显示，下季度各部门人才密度将呈现稳步上升趋势。'}
                      </p>
                    </div>
                  ...
                  <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    return (
                                      <div key={item.name} className="group/bar">
                                        ...
                                        <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden relative">
                                          <div
                                            style={{
                                              width: `${data[0]?.value ? (item.value / data[0].value) * 100 : 0}%`,
                    ...
                    <div className="bg-gradient-to-br from-indigo-900/40 to-slate-900/40 backdrop-blur-md p-6 rounded-3xl border border-indigo-500/20 flex flex-col relative overflow-hidden">
                      ...
                      <h3 className="text-sm font-bold text-white flex items-center gap-2 mb-4 relative z-10">
                        <Target size={16} className="text-indigo-400" /> 决策建议 (Actions)
                      </h3>
                      <div className="flex-1 space-y-3 relative z-10">
                        {aiReportConfig.suggestions?.slice(0, 3).map((sugg, i) => (
                          <div key={i} className="flex gap-3 group cursor-pointer">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                <div className="text-xs font-bold text-indigo-100 group-hover:text-white transition-colors">
                                  {sugg.title}
                                </div>
                                {sugg.priority && (
                                  <span className="text-[9px] px-1.5 py-0.5 bg-indigo-500/20 rounded text-indigo-300 border border-indigo-500/30">
                                    {sugg.priority}
                                  </span>
                                )}
                              </div>
              ...
              <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
                <div className="relative overflow-hidden p-6 rounded-3xl bg-gradient-to-r from-[#0b1224] via-[#111c33] to-[#0d1a2b] border border-white/10 shadow-[0_20px_60px_-25px_rgba(0,0,0,0.8)]">
                  ...
                  <div className="relative flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <button
                      onClick={() =>
                        generateDynamicAnalysis(aiParams?.query || '综合人力画像', employees, departments, dataSchema, recruitmentTrend)
                      }
                  ...
                  {/* 关键：这里原来被你 merge 时“重复插入”了一段 map，我已去重，只保留一份 */}
                  ...
                  <div className="relative mt-4 text-xs text-slate-300 flex items-center gap-2">
                    ...
                    <span className="text-indigo-100 font-semibold">{baselineStats.avgTenure} 年</span>，可对比各部门梯队稳定性。
                  </div>
                </div>
